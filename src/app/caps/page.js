...
const getCaps = async (context) => {
    await connectDB();
    const products = await Product.find({ category: "cap" });
    return { products }
}

const Caps = async () => {
    const { products } = await getCaps();
                ...
                <div className="container p-8 mx-auto">
                    <div className="flex flex-wrap m-0 justify-center align-middle">
                        <div className="text-center w-full my-4">
                            <span className='text-pink-500 font-bold text-xl'>Caps</span>
                        </div>
                            ...
                            return <div key={item._id} className="lg:w-1/5 md:w-1/2 p-4 w-5/6 m-auto shadow-xl md:m-2">
                                <Link href={`/product/${item.slug}`} >
                                    ...
                                    <div className="text-center mt-4 w- ">
                                        <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">Cap</h3>
                        ...
                        <span className='text-gray-500 font-bold text-lg'>Currently Caps are Out of Stock</span>
                    </div>
                        }
                    </div>
                </div>
